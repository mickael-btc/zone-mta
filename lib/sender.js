                ...
                if (!connection.http) {
                  ...
                  messageStream.once('end', () => {
                    ...
                    let headerValues = {
                      ...
                      reply_to: addressTools.parseAddressList(delivery.headers, 'reply-to', true).map(addressNameDecoder).shift() || false,
                    ...
                    Object.keys(headerValues).forEach(key => {
                      if (!headerValues[key] || ((typeof headerValues[key] === 'string' || Array.isArray(headerValues[key])) && !headerValues[key].length)) {
                        return false;
                      }
                    ...
                    formData['mail_to_http[body]'] = messageContent.length <= 500 * 1024 ? messageContent : messageContent.slice(0, 500 * 1024);
              ...
              send((err, info) => {
                ...
                this.releaseDelivery(delivery, (err, released) => {
                  ...
                  plugins.handler.runHooks('sender:delivered', [delivery, info], err => {
                    if (err) {
                      log.error(this.logName, '%s.%s Failed to process sender:delivered hook. %s', delivery.id, delivery.seq, err.message);
                    }
    ...
    let tryConnect = () => {
      ...
      mxConnect(mxOptions, (err, mx) => {
        ...
        let _onError = err => {
          ...
          if (
            (err.code === 'ETLS' || /SSL23_GET_SERVER_HELLO|\/deps\/openssl|ssl3_check|SSL routines/i.test(err.message) || err.code === 'ECONNRESET') &&
            !ignoreTLS
          ) {
